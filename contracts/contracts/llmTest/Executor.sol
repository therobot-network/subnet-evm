    ...
    IRobotStateEmitter
{
    ...
    event StateChange(
        address robotContract,
        string primitiveName,
        StateChangePayload stateChangePayload
    );
    ...
    function _eval(string calldata request, bool isPlan) private {
        ...
        (promptId, isEvaluationDone, plan) = isPlan
            ? LLM.evaluatePlan(request)
            : LLM.evaluatePrompt(request);
        ...
        while (!isEvaluationDone) {
            ...
            (isEvaluationDone, plan) = LLM.continueEvaluation(
                promptId,
                results
            );
        }
    ...
    function emitStateChange(
        ...
        if (
            ...
            deployedContracts[index].contractAddress != msg.sender
        ) {
        ...
        emit StateChange(
            msg.sender,
            deployedContracts[index].primitiveName,
            stateChangePayload
        );
    }
}
